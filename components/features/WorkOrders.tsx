...
export function WorkOrders() {
  ...
  const { options: filterOptionsData, counts: filterCounts } = useFilterOptions(workOrders, {
    status: (wo) => wo.status,
    priority: (wo) => wo.priority,
    serviceCategory: (wo) => wo.serviceCategory,
    propertyType: (wo) => wo.propertyType,
    client: (wo) => wo.clientName,
    assignedTechnician: (wo) => wo.assignedTechnician || '',
  });

  // Build filter options object
  const filterOptions = useMemo(() => ({
    clients: filterOptionsData.client || [],
    technicians: filterOptionsData.assignedTechnician?.filter(Boolean) || [],
    statusCounts: filterCounts.status || {},
    priorityCounts: filterCounts.priority || {},
    categoryCounts: filterCounts.serviceCategory || {},
    propertyTypeCounts: filterCounts.propertyType || {},
    clientCounts: filterCounts.client || {},
    technicianCounts: filterCounts.assignedTechnician || {},
  }), [filterOptionsData, filterCounts]);

  // Get cost range stats
  const costStats = useNumericRangeStats(workOrders, (wo) => wo.estimatedCost);
  ...
  const filteredData = useTableFilters({
    ...
    dateFields: {
      requestDate: (wo) => wo.requestDate,
      dueDate: (wo) => wo.dueDate,
    },
    numericFields: {
      cost: (wo) => wo.estimatedCost,
    },
  });
  ...
  // Use pagination hooks for each inbox tab
  const awaitingResponsePagination = usePagination(inboxData.awaitingResponse, {
    itemsPerPage,
    initialPage: awaitingResponsePage,
  });

  const myWorkOrdersPagination = usePagination(inboxData.myWorkOrders, {
    itemsPerPage,
    initialPage: myWorkOrdersPage,
  });

  const rejectedPagination = usePagination(inboxData.rejected, {
    itemsPerPage,
    initialPage: rejectedPage,
  });

  // Sync pagination state
  useEffect(() => {
    setAwaitingResponsePage(awaitingResponsePagination.page);
  }, [awaitingResponsePagination.page]);

  useEffect(() => {
    setMyWorkOrdersPage(myWorkOrdersPagination.page);
  }, [myWorkOrdersPagination.page]);

  useEffect(() => {
    setRejectedPage(rejectedPagination.page);
  }, [rejectedPagination.page]);

  useEffect(() => {
    setItemsPerPage(awaitingResponsePagination.itemsPerPage);
  }, [awaitingResponsePagination.itemsPerPage]);

  // Paginated data for each tab
  const paginatedInboxData = useMemo(() => ({
    awaitingResponse: {
      items: awaitingResponsePagination.paginatedData,
      totalPages: awaitingResponsePagination.totalPages,
      totalItems: awaitingResponsePagination.totalItems,
    },
    myWorkOrders: {
      items: myWorkOrdersPagination.paginatedData,
      totalPages: myWorkOrdersPagination.totalPages,
      totalItems: myWorkOrdersPagination.totalItems,
    },
    rejected: {
      items: rejectedPagination.paginatedData,
      totalPages: rejectedPagination.totalPages,
      totalItems: rejectedPagination.totalItems,
    },
  }), [awaitingResponsePagination, myWorkOrdersPagination, rejectedPagination]);

  // Action handlers
  const handleMoveToInProgress = (workOrderId: string) => {
  ...
  const handleMoveToRejected = (workOrderId: string) => {
    setWorkOrders(prev => prev.map(wo => 
      wo.id === workOrderId 
  ...
  const handleArchive = (workOrderId: string) => {
                ...
                <TabsContent value="awaiting-response" className="mt-0 space-y-4">
                  {paginatedInboxData.awaitingResponse.items.length > 0 ? (
                    <>
                      ...
                      {/* Pagination Controls */}
                      <InboxPagination
                        currentPage={awaitingResponsePagination.page}
                        totalPages={awaitingResponsePagination.totalPages}
                        itemsPerPage={awaitingResponsePagination.itemsPerPage}
                        totalItems={awaitingResponsePagination.totalItems}
                        onPageChange={awaitingResponsePagination.setPage}
                        onItemsPerPageChange={(newItemsPerPage) => {
                          awaitingResponsePagination.setItemsPerPage(newItemsPerPage);
                          awaitingResponsePagination.setPage(0);
                        }}
                      />
                    </>
                  ) : (
                ...
                <TabsContent value="my-work-orders" className="mt-0 space-y-4">
                  {paginatedInboxData.myWorkOrders.items.length > 0 ? (
                    <>
                      ...
                      {/* Pagination Controls */}
                      <InboxPagination
                        currentPage={myWorkOrdersPagination.page}
                        totalPages={myWorkOrdersPagination.totalPages}
                        itemsPerPage={myWorkOrdersPagination.itemsPerPage}
                        totalItems={myWorkOrdersPagination.totalItems}
                        onPageChange={myWorkOrdersPagination.setPage}
                        onItemsPerPageChange={(newItemsPerPage) => {
                          myWorkOrdersPagination.setItemsPerPage(newItemsPerPage);
                          myWorkOrdersPagination.setPage(0);
                        }}
                      />
                    </>
                  ) : (
                  ...
                  {paginatedInboxData.rejected.items.length > 0 ? (
                    <>
                      ...
                      <InboxPagination
                        ...
                        onItemsPerPageChange={(newItemsPerPage) => {
                          rejectedPagination.setItemsPerPage(newItemsPerPage);
                          rejectedPagination.setPage(0);
                        }}
                      />
                    </>
                  ) : (
